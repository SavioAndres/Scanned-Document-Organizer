...
public class MaskField extends TextField {
	...
	/**
	 * если маска должна отображать символы которые зарезервированы для маски, то
	 * задается дополнительная подсказка где символ маски, а где просто символ
	 */
	...
	private void rebuildObjectMask() {
		...
		for (int i = 0; i < getMask().length(); i++) {
			...
			if (getWhatMask() != null && i < getWhatMask().length()) {
				...
				// и если символ не находится среди символов маски - то это считается простым
				// литералом
